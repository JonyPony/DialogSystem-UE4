// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "DialogueEditor/Nodes/BaseNode/PZ_DialogueSNode_Base.h"


class UPZ_DialogueEdNode_RunService;
class UPZ_DialogueRRunServiceNode;



class SPZ_DialogueSNode_RunService : public SPZ_DialogueSNode_Base
{

	typedef SPZ_DialogueSNode_Base Super;
	typedef SPZ_DialogueSNode_RunService Self;


public:

	SLATE_BEGIN_ARGS(SPZ_DialogueSNode_RunService) {}
	SLATE_END_ARGS()

	void Construct(const FArguments& InArgs, UPZ_DialogueEdNode_RunService* InNode);



public:

	virtual FText GetNodeName() const override;

	virtual FLinearColor GetNodeTitleColor()  const override;
	virtual FLinearColor GetNodeBackgroundColor()  const override;
	
	virtual const FSlateBrush* GetBackgroundImage()  const override;
	virtual const FSlateBrush* GetNodeImage()  const override;
	virtual const FSlateBrush* GetRoleImage() const override;
	virtual const FLinearColor GetRoleImageColor() const override;



public:

	UPZ_DialogueEdNode_RunService* EdRunServiceNode = nullptr;
	UPZ_DialogueRRunServiceNode* RRunServiceNode = nullptr;

};
